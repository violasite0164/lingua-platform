...
export function LessonEditorCard({
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      const subFd = subFormRef.current
        ? new FormData(subFormRef.current)
        : new FormData();
      const subAccess = parseSubscriptionAccessFromForm(subFd);
      const res = await updateLessonAction(lesson.id, {
        title,
        description,
        is_preview: isPreview,
        xp_reward: Number(xpReward) || 0,
        sub_access_override: subFd.get('sub_access_override') === 'on',
      ...
      <CardContent className="space-y-4">
        ...
        <form ref={subFormRef}>
          <SubscriptionAccessFields
            ...
            entityBasic={lesson.sub_basic_free}
            entityPro={lesson.sub_pro_free}
