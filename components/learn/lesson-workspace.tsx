...
import { CourseRoadmapList } from '@/components/courses/course-roadmap-list';
import {
  buildCourseRoadmap,
} from '@/lib/course-quiz/roadmap';
import { canWatchLessonVideo } from '@/lib/billing/subscription-access';
import {
  canAccessLessonInCourse,
  canAccessQuizInCourse,
  findRoadmapIndexForLesson,
  quizGateAfterLesson,
...
function roadmapHref(courseId: string, item: CourseRoadmapItem): string {
  if (item.kind === 'lesson') {
...
export function LessonWorkspace({
  ...
  const canWatch = canWatchLessonVideo(lesson, course, course.subscription_tier ?? 'free');
  ...
  const roadmapItems = buildCourseRoadmap(course.lessons, course.quizzes ?? []);
  const roadmapIndex = findRoadmapIndexForLesson(roadmapItems, lesson.id);
  const lessonIndex = course.lessons.findIndex((l) => l.id === lesson.id);

  const prevHref =
    roadmapIndex > 0
      ? roadmapHref(course.id, roadmapItems[roadmapIndex - 1]!)
      : null;
  const nextHref =
    roadmapIndex >= 0 && roadmapIndex < roadmapItems.length - 1
      ? roadmapHref(course.id, roadmapItems[roadmapIndex + 1]!)
      : null;

  const quizGateHref =
    lessonCompleted && canWatch
      ? quizGateAfterLesson(course, lesson.id)
      : null;

  function canNavigateHref(href: string | null): boolean {
    if (!href) return false;
    const quizMatch = href.match(/\/quiz\/([^/]+)$/);
    if (quizMatch) {
      return canAccessQuizInCourse(course, quizMatch[1]!);
    }
    const lessonId = href.split('/').pop();
    if (!lessonId) return false;
    return canAccessLessonInCourse(course, lessonId);
  }
    ...
    <div className="grid min-w-0 gap-6 lg:grid-cols-3 lg:gap-8">
      <div className="min-w-0 space-y-4 sm:space-y-6 lg:col-span-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {prevHref && canNavigateHref(prevHref) ? (
          ...
          {quizGateHref ? (
            <Button size="sm" asChild>
              <Link href={quizGateHref}>進行課堂測驗</Link>
            </Button>
          ) : null}
          {nextHref && canNavigateHref(nextHref) && !quizGateHref ? (
            <Button variant="outline" size="sm" asChild>
              <Link href={nextHref}>
                下一項
