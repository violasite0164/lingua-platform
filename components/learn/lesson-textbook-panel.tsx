...
function textbookDownloadApiUrl(textbookId: string): string {
  return `/api/lesson-textbooks/${textbookId}/download`;
}

async function fetchTextbookBlob(tb: LessonTextbook): Promise<Blob> {
  const res = await fetch(textbookDownloadApiUrl(tb.id), { credentials: 'include' });
  if (!res.ok) {
    let message = '無法取得檔案';
    try {
...
export function LessonTextbookPreviewPopup({
  ...
  async function runTextbookAction(action: 'download' | 'print') {
    ...
    try {
      ...
      window.alert(err instanceof Error ? err.message : '下載失敗，請稍後再試');
    } finally {
      setActionBusy(false);
    }
  }
