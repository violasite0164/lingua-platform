...
export async function listProfileInboxMessages(): Promise<
...
export async function getProfileInboxUnreadCount(): Promise<number> {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
...
export async function markProfileInboxRead(messageIds?: string[]): Promise<
...
export async function claimProfileInboxStaminaPack(messageId: string): Promise<
...
> {
  ...
  if (!canClaimStaminaPack(message)) {
