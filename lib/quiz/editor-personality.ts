import { getQuizStageRemarkOutcome, type QuizStageRemarkOutcome } from '@/lib/quiz/constants';
...
export function generateEditorRemark(
  ...
  const stageOutcome = getQuizStageRemarkOutcome(score100);
  ...
  const stageRemarkPools: Record<
    QuizStageRemarkOutcome,
    Record<QuizEditorPersonality, Cand[]>
  > = {
    pass: {
      gentle: [
        { t: '通關成功！總分 {score100}/100，這一關你撐過去了，值得好好慶祝。' },
        { t: '你過關了：答對 {correct}/{total}，總分 {score100}。節奏穩下來，下一關也繼續相信自己。' },
        { t: 'STAGE CLEAR！{avgDifficultyLabel} 這關被你拿下，{score100}/100 是努力的證明。' },
        { t: '通關成功——平均 {avgSec} 秒／題，又快又穩的感覺出來了。' },
        { t: '這局順利過關，答對分 {accPts}/65、速度分 {speedPts}/35 都幫了大忙。' },
        { t: '恭喜通關！把這份手感留住，下一關會更有信心。' },
      ],
      toxic: [
        { t: '通關成功，{score100}/100——唔好即刻鬆懈，下一關唔會放過你。' },
        { t: '你過關了：答對 {correct}/{total}，總分 {score100}。算你有料，但唔好得戚到飛起。' },
        { t: 'STAGE CLEAR：{avgDifficultyLabel} 都俾你啃落——雖然我唔想承認，但你今局合格。' },
        { t: '總分 {score100} 過關——平均 {avgSec} 秒／題，今次唔係運氣，係真係有做功課。' },
        { t: '通關成功！答對分 {accPts}/65 同速度分 {speedPts}/35 夠晒交差——繼續卷，唔好停。' },
        { t: '你過到關啦——下一關難啲，睇下你係咪只係今次僥倖。' },
      ],
    },
    near_pass: {
      gentle: [
        { t: '差一點就通關了，總分 {score100}/100——真的很接近，再練一兩次很有機會。' },
        { t: '還差一點點：答對 {correct}/{total}，總分 {score100}。把錯題看懂，下一局很可能就過關。' },
        { t: '這次還沒過關，但 {score100}/100 已經很接近，別灰心。' },
        { t: '差一點就通關成功——平均 {avgSec} 秒／題，節奏 OK，再補穩幾題就夠。' },
        { t: '離過關只差一步：答對分 {accPts}/65 或速度分 {speedPts}/35 再拉高少少就穩。' },
        { t: '很接近了！保持今天這個狀態，下一局通關機會很大。' },
      ],
      toxic: [
        { t: '差一點就過關——總分 {score100}/100，你係專登喺門口彈返轉頭。' },
        { t: '未通關，但差少少：答對 {correct}/{total}。再錯少題你就唔使聽我嘈。' },
        { t: '{score100}/100——你而家狀態係「差一啲」，唔係「差好遠」。' },
        { t: '差一點通關：平均 {avgSec} 秒／題。快啲啱多幾題，唔好浪費呢個勢。' },
        { t: '答對分 {accPts}/65、速度分 {speedPts}/35——再加少少就過關，唔好扮唔知。' },
        { t: '今局 FAIL 但好接近；下一局再咁就唔係運氣，係實力。' },
      ],
    },
    fail: {
      gentle: [
        { t: '今次未能通關，總分 {score100}/100，沒關係，我們從解析把弱點一題題補回來。' },
        { t: '答對 {correct}/{total}，總分 {score100}——這關還沒過，但每錯一題都是下次的養分。' },
        { t: '通關失敗不代表失敗：{avgDifficultyLabel} 有難度，願意打完一整局就很棒。' },
        { t: '平均 {avgSec} 秒／題：先求讀懂題目，分數會跟著慢慢上來。' },
        { t: '答對分 {accPts}/65、速度分 {speedPts}/35 告訴我們該練哪裡——再來一局會更好。' },
        { t: '這局沒過關，休息一下，帶著錯題筆記再挑戰一次。' },
      ],
      toxic: [
        { t: '通關失敗，總分 {score100}/100——你唔係差一啲，你係差成個宇宙。' },
        { t: '答對 {correct}/{total}，總分 {score100}。呢關你未過，題庫都替你尷尬。' },
        { t: 'STAGE FAIL：{avgDifficultyLabel} 都未啃得落，先諗下係題難定係你鬆。' },
        { t: '平均 {avgSec} 秒／題——諗幾耐都唔幫你過關，要啱先得。' },
        { t: '答對分 {accPts}/65、速度分 {speedPts}/35——兩邊都要加料，唔好再藉口。' },
        { t: '今局未過關：回去睇解析，唔好即刻開下一關裝冇事。' },
      ],
    },
  };
  ...
  const pool: Cand[] = [
    ...stageRemarkPools[stageOutcome][personality],
    ...byScore[sTier].slice(0, 2),
  ];
